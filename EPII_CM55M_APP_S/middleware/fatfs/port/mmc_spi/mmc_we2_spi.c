...
static bool wait_spi_completed(uint32_t data_count)
{
    ...
    if (Driver_SPI0.GetStatus().data_lost)
    {
        return false;
    }
...
)
{
    ...
    while (block_num)
    {
        ...
        if (!wait_spi_completed(512))
        {
            return btr - block_num * 512;
        }
        block_num--;
        buff += 512;
    }
    ...
    if (last_btr)
    {
        ...
        if (!wait_spi_completed(last_btr))
        {
            return btr - last_btr;
        }
    }
...
)
{
    ...
    if (last_btx)
    {
        ...
        if (!wait_spi_completed(last_btx))
        {
            return btx - last_btx;
        }
    }
...
)
{
    ...
    if(rcvr_spi_multi(buff, btr) != btr){/* Store trailing data to the buffer */
        return 0; 
    }; 
    xchg_spi(0xFF);
...
)
{
    ...
    {                              /* Send data if token is other than StopTran */
        if(xmit_spi_multi(buff, 512) != 512){
            return 0;
        }; /* Data */
        xchg_spi(0xFF);
