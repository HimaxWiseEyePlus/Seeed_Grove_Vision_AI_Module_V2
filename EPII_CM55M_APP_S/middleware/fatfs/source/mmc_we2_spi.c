...
static void SSPI_CS_GPIO_Output_Level(bool setLevelHigh)
{
    hx_drv_gpio_set_out_value(GPIO16, (GPIO_OUT_LEVEL_E) setLevelHigh);
}

static void SSPI_CS_GPIO_Pinmux(bool setGpioFn)
{
#ifdef NSC
    if (setGpioFn)
        veneer_sys_set_PB5_pinmux(SCU_PB5_PINMUX_GPIO16);
    else
        veneer_sys_set_PB5_pinmux(SCU_PB5_PINMUX_SPI_M_CS_1);
#else
    if (setGpioFn)
...
static void SSPI_CS_GPIO_Dir(bool setDirOut)
{
    if (setDirOut)
...
#define MMC_WP()    0
#define MMC_CD()    1
#define CS_HIGH()   Driver_SPI0.Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_INACTIVE);
#define CS_LOW()    Driver_SPI0.Control(ARM_SPI_CONTROL_SS, ARM_SPI_SS_ACTIVE);
...
static void power_on(void)
{
    /* Enable SPI function */
#ifdef NSC
    veneer_sys_set_PB2_pinmux(SCU_PB2_PINMUX_SPI_M_DO_1);
    veneer_sys_set_PB3_pinmux(SCU_PB3_PINMUX_SPI_M_DI_1);
    veneer_sys_set_PB4_pinmux(SCU_PB4_PINMUX_SPI_M_SCLK_1);
    veneer_sys_set_PB5_pinmux(SCU_PB5_PINMUX_SPI_M_CS_1);
#else
    ...
    hx_drv_scu_set_PB5_pinmux(SCU_PB5_PINMUX_SPI_M_CS_1);
#endif
    Driver_SPI0.Initialize(NULL);
    Driver_SPI0.PowerControl(ARM_POWER_FULL);
    int32_t ret = Driver_SPI0.Control(ARM_SPI_MODE_MASTER | 
                                      ARM_SPI_CPOL0_CPHA0 |
                                      ARM_SPI_MSB_LSB |
    ...
    Driver_SPI0.Control(ARM_SPI_SET_DEFAULT_TX_VALUE, 0xFFFFFFFF);
}
...
{
    while (Driver_SPI0.GetDataCount() < data_count || Driver_SPI0.GetStatus().busy);
}
