...
// Function to write a signed rational value (2 int32 values)
static uint32_t write_srational(unsigned char *buffer, uint32_t offset, int32_t num, int32_t denom)
{
    WRITE32LE(buffer + offset, num);
    WRITE32LE(buffer + offset + 4, denom);
    return offset + 8;
