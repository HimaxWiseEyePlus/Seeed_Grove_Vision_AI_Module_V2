...
#include "cis_file.h"	// Some register names
...
// Name of file containing extra HM0360 parameters
// File is created using the python script in _Tools
#define EXTRAHM0360PARAMETERS	"0:/hm0360_config.bin"
...
static void enterDPD(void) {
	...
	// (0x8030 gives about 1s)
	// try 1 frame?
	cisdp_sensor_set_mode(MODE_SW_NFRAMES_SLEEP, 2, 0x8030);
...
void main_task(void *pvParameters) {
	...
	if ((wakeup_event == PMU_WAKEUP_NONE) && (wakeup_event1 == PMU_WAKEUPEVENT1_NONE)) {
...
#if ( SUPPORT_FATFS == 1 )
		imageSequenceNumber = fatfs_init();		// Read the previous sequence number from a file
#endif
    	...
    	if (cameraPresent) {
    		...
    		// Initialise the HM0360 registers and start the first image capture
#if 1
            app_start_state(APP_STATE_ALLON);
#else
    		cisdp_sensor_init(true);

    		//cisdp_sensor_start();
    		// default register settings for 0x3029, 0x302a = 0x1560
    		cisdp_sensor_set_mode(MODE_SW_NFRAMES_SLEEP, 22, 0x1560);

#if ( SUPPORT_FATFS == 1 )
		if ((imageSequenceNumber > 0) && (cis_file_process(EXTRAHM0360PARAMETERS) == HX_CIS_NO_ERROR)) {
			xprintf("Wrote new values from %s\n", EXTRAHM0360PARAMETERS);
		}
#endif
    		// the next 2 calls are in cisdp_sensor_start() - what do they do?
		...
		XP_WHITE;
		xprintf("We will take %d pictures at %dms intervals before sleeping starting at image %d\n",
