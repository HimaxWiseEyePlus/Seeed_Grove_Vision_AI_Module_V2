...
static void dp_app_cv_efficientnet_lite0_class_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
...
void app_start_state(APP_STATE_E state)
{
	...
	dp_var_int();
	if(state == APP_STATE_ALLON_EFFICIENTNET_LITE0_CLASS)
	{

		if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_efficientnet_lite0_class_eventhdl_cb, 4, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_1X) < 0)
        {
        	xprintf("\r\nDATAPATH Init fail\r\n");
        	APP_BLOCK_FUNC();
        }
...
void model_change() {
	...
	if(g_use_case==0) {
		...
		cv_efficientnet_lite0_class_deinit();
	}
#endif
...
#endif
	if(g_use_case == 0) {
				xprintf("efficientnet lite0 classification \n");
#ifdef EN_ALGO
		cv_efficientnet_lite0_class_init(true, true, EFFICIENTNET_LITE0_CLASSIFICATION_FLASH_ADDR);
#endif
	    app_start_state(APP_STATE_ALLON_EFFICIENTNET_LITE0_CLASS);
	}
	return 0;
}
